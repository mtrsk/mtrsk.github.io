...
There is also a couple of stadards we try to follow when doing this project, all
...
~rvm~, ...) but capable of installinng anything availiable on [[https://search.nixos.org/packages][nixpkgs]] and
much more.
...
shell just by ~cd~-ing into the project's directory. Here's an example of my
~.envrc~:
...
to have a postgres service running to debug and test locally. However, when we
talk about releases, things change, we need to find a way to properly build the
server.
...
add in our ~rebar3~ config to benefit from the Nix integration:
...
development environment for the developers, to re-using some of the stuff in CI,
to later repurpose some of the flows for pure builds, that later got repursed
into our containers, all by leveraging the **same tool**. I wish modern devops
was more about that, but it seems it'll take time for people to realize that
**immutability**, **composition** and **functional programming** can go hand in hand
and give us a better experience than one can find in most other solution (built
by trillion dollar companies). Luckilly, Nix is [[https://www.youtube.com/watch?v=FJVFXsNzYZQ][gaining some traction]] and more
...
- [ ] I have yet to learn how to deploy a production-ready erlang system
